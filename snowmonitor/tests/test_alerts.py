...
class EvaluateTests(unittest.TestCase):
    def test_no_alerts_on_healthy(self):
        healthy = {
    ...
    def test_budget_pacing_proactive(self):
        result = alerts.evaluate({"mtd_spend_usd": 20000, "days_elapsed": 10, "days_in_month": 30})
    ...
    def test_failed_tasks_reactive(self):
    ...
    def test_spend_spike(self):
        below = alerts.evaluate({"today_spend_usd": 120, "avg_daily_spend_usd_7d": 100})
        above = alerts.evaluate({"today_spend_usd": 200, "avg_daily_spend_usd_7d": 100})
    ...
    def test_sorted_by_severity(self):
        result = alerts.evaluate({"failed_task_runs": 6, "queued_queries": 100})
...
class AlertObjectSqlTests(unittest.TestCase):
    def test_shape(self):
        sql = alerts.build_alert_object_sql(
            name="Failed Tasks", warehouse="MONITOR_WH", schedule_minutes=15,
