...
-- Creates the cost mart (so the app reads pre-aggregated data instead of scanning
-- ACCOUNT_USAGE on every page load), the alert ledger, the app error log, and a
-- scheduled task that refreshes the mart.
--
...
BEGIN
    ...
    WHEN MATCHED THEN UPDATE SET
        compute_credits = s.compute_credits,
        cloud_services_credits = s.cloud_services_credits,
    ...
    -- Allocated cost by user/role/database (elapsed-time share of warehouse-hour credits)
    ...
    WHEN MATCHED THEN UPDATE SET
        allocated_credits = s.allocated_credits, query_count = s.query_count, loaded_at = CURRENT_TIMESTAMP()
    WHEN NOT MATCHED THEN INSERT
        (usage_date, warehouse_name, user_name, role_name, database_name, allocated_credits, query_count)
        VALUES (s.usage_date, s.warehouse_name, s.user_name, s.role_name, s.database_name, s.allocated_credits, s.query_count);

    RETURN 'SnowMonitor mart refreshed';
END;
$$;

-- Backfill 90 days now, then schedule a refresh of the trailing 3 days every hour.
...
-- Optional: retain ~400 days in the mart.
-- DELETE FROM MART_WAREHOUSE_COST_DAILY WHERE usage_date < DATEADD('day', -400, CURRENT_DATE());
-- DELETE FROM MART_QUERY_ATTR_DAILY     WHERE usage_date < DATEADD('day', -400, CURRENT_DATE());
