...
Warehouse cost is exact (warehouse-hour metering); user/role/db/application cost is
allocated by query elapsed-time share and labeled as such.
"""
...
def task_runs_sql(days: int, company_name: str, top: int = 200) -> str:
    ...
    return f"""
    SELECT
        name AS TASK, database_name AS DATABASE, schema_name AS SCHEMA, state AS STATE,
        scheduled_time AS SCHEDULED_TIME, completed_time AS COMPLETED_TIME,
        DATEDIFF('second', query_start_time, completed_time) AS DURATION_SEC,
        error_code AS ERROR_CODE, error_message AS ERROR_MESSAGE, root_task_id AS ROOT_TASK_ID
    FROM {AU}.TASK_HISTORY
    WHERE scheduled_time >= {_window(days)} AND database_name IS NOT NULL {scope}
...
def task_graph_sql(days: int, company_name: str, top: int = 100) -> str:
    ...
    return f"""
    WITH runs AS (
        SELECT COALESCE(root_task_id, name) AS graph_key, name, database_name, schema_name, state,
               DATEDIFF('second', query_start_time, completed_time) AS dur_sec, scheduled_time
        FROM {AU}.TASK_HISTORY
        WHERE scheduled_time >= {_window(days)} AND database_name IS NOT NULL {scope}
    )
    SELECT
        graph_key AS GRAPH, ANY_VALUE(database_name) AS DATABASE,
        COUNT(DISTINCT name) AS TASKS_IN_GRAPH, COUNT(*) AS TOTAL_RUNS,
        SUM(IFF(state = 'FAILED', 1, 0)) AS FAILED_RUNS,
        SUM(IFF(state = 'SUCCEEDED', 1, 0)) AS SUCCEEDED_RUNS,
        ROUND(AVG(dur_sec), 1) AS AVG_DURATION_SEC, MAX(scheduled_time) AS LAST_RUN
    FROM runs
    GROUP BY graph_key
...
def failed_logins_sql(days: int, company_name: str, top: int = 200) -> str:
    ...
    return f"""
    SELECT
        event_timestamp AS EVENT_TIME, user_name AS USER, reported_client_type AS CLIENT,
        error_message AS REASON, client_ip AS CLIENT_IP,
        first_authentication_factor AS FIRST_FACTOR, second_authentication_factor AS SECOND_FACTOR
    FROM {AU}.LOGIN_HISTORY
    WHERE event_timestamp >= {_window(days)} AND is_success = 'NO' {scope}
...
def users_without_mfa_sql(company_name: str, days: int = 90) -> str:
    ...
    Excludes users who actually authenticate via SSO/OAuth or key-pair, and
    prioritizes users with a confirmed recent password login that used no second
    factor. NOTE: LOGIN_HISTORY factor strings vary by account; the SSO/key-pair
    set below covers common values and is easy to extend (see VALIDATION.md).
    """
    ...
    SELECT
        u.name AS USER, u.email AS EMAIL, u.default_role AS DEFAULT_ROLE,
        u.last_success_login AS LAST_LOGIN,
        CASE WHEN pw.u IS NOT NULL THEN 'Confirmed: password login, no MFA'
             ELSE 'Potential: password enabled, no MFA enrolled' END AS RISK_BASIS
    FROM {AU}.USERS u
    LEFT JOIN pw_only pw ON pw.u = UPPER(u.name)
    LEFT JOIN federated f ON f.u = UPPER(u.name)
    WHERE u.deleted_on IS NULL AND u.disabled = FALSE AND u.has_password = TRUE
      AND COALESCE(u.ext_authn_duo, FALSE) = FALSE
      AND COALESCE(u.has_rsa_public_key, FALSE) = FALSE
      AND f.u IS NULL
      {scope}
...
def warehouse_names_sql() -> str:
    """Distinct warehouse names seen in metering (for the Controls picker)."""
    return f"""
    SELECT DISTINCT warehouse_name AS WAREHOUSE
    FROM {AU}.WAREHOUSE_METERING_HISTORY
